...
enum OnboardingViews {
  ...
  @ViewBuilder
  static func textField(_ text: Binding<String>,
                        ...
                        onFocusChange: Optional<(Bool) -> Void> = nil,
                        onTextChange: Optional<(String) -> Void> = nil) -> some View {
    FocusedTextField(text: text, error: error ?? .constant(""),
                     isFocused: isFocused, placeholder: placeholder,
                     font: font, placeholderFont: placeholderFont,
                     color: color, placeholderColor: placeholderColor, focusedColor: focusColor, isSecure: isSecure,
                     onFocusChange: onFocusChange, onTextChange: onTextChange)
  }
  ...
  static var links: some View {
    HStack(alignment: .center, spacing: 2) {
      ...
      Link("UI Faces".local, destination: .pictures)
        .foregroundColor(.green).font(.poppinsBold(12))
      Text(". Please visit ".local).font(.poppins(12))
      Link("FAQ".local, destination: .faq)
        .foregroundColor(.green).font(.poppinsBold(12))
...
private extension URL {
  static var faq: URL {
    URL(string: "https://web.archive.org/web/20160811185628/http://uifaces.com/faq")!
  }

  static var pictures: URL {
    URL(string: "http://uifaces.com/")!
  }
}
